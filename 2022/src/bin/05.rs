...
fn main() {
    let input = include_str!("./data.txt")
        ...
        .map(|(p1, p2)| {
            (
                p1.lines().map(String::from).collect::<Vec<String>>(),
                p2.lines().map(String::from).collect::<Vec<_>>(),
            )
        })
        .unwrap();
    ...
    let moves: Vec<_> = input
        .1
        .iter()
        .map(|n| n.replace("move ", ""))
        .map(|n| n.replace("from", "to"))
        .map(|n| {
            n.split(" to ")
    ...
    println!(
        ...
        crate_stacks
            .iter()
            .map(|(_, v)| v.last())
            .fold("".to_string(), |acc, value| acc + value.unwrap())
    );
    println!(
        "Task 2 result: {:}",
        crate_stacks2
